...
// leetcode hard solved here - serialize , deserialize bt
    destringify(data){
        let node = [];
        if(data[this.ind]==='\''){
            this.ind++;
...
// doing encryption here
    ...
    // interesting part 
       decode(data){
        data = data.split('\n');
        if(data.length===4){
            // Handling new line - sexy challenge and edge case taken care of (tell the interviewer)
            data[0] = data[0] + '\n' + data[1];
            data[1] = data[2];
            data[2] = data[3];
            data.pop();
